...
export default function Details() {
  ...
  const consultationIdString = Array.isArray(idConsulation)
    ? idConsulation[0]
    : idConsulation;

  const [patient, setPatient] = useState<PatientInformations | null>(null);
  const [consultation, setConsultation] = useState<Consultation[] | null>(null);
  const [prevsConsultations, setPrevsConsultations] = useState<
    Consultation[] | null
  >(null);
  ...
  useEffect(() => {
    if (consultationIdString) {
      const fetchConsultationAndPatient = async () => {
        try {
          console.log(
            "1. Buscando consulta atual por ID:",
            consultationIdString
          );
          const response =
            await consultationService.getConsultationById(consultationIdString);
          const currentConsultation: Consultation = response;
          console.log(
            "2. Consulta Atual (currentConsultation):",
            currentConsultation
          );
          ...
          if (currentConsultation) {
            ...
            const patientResponse =
              await patientService.getPatientInfoById(patientId);
            const foundPatient: PatientInformations = patientResponse;
            setPatient(foundPatient);

            console.log("3. Buscando histórico para o paciente:", patientId);
            const prevsConsults =
              await consultationService.getPrevsConsultation(
                patientId,
                currentConsultation
              );
            console.log(
              "4. Consultas Anteriores Encontradas (prevsConsults):",
              prevsConsults
            );
  ...
  // Calcula a imagem padrão caso não seja DOG
        ...
        <div className="flex justify-between gap-x-12">
          <div className="">
            ...
            <div className="mt-10 flex flex-col w-[39.4375rem] h-[8.625rem] items-center justify-center shadow rounded-3xl border-[1px] gap-6 p-6">
              ...
              <NovaConsultaModal id={patient.id} />
            </div>
          </div>
